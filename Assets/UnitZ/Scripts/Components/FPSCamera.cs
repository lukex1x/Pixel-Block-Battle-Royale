    ...
    void Update()
    {
        ...
        if ((IsThirdView && !zooming) || (IsThirdView && (zooming && !hideWhenScoping)))
        {
            ...
            if (IsThirdView)
            {
                ...
                hit = Physics.RaycastAll(this.transform.position + (this.transform.right * ThirdViewOffset.x * ThirdViewInvert), -this.transform.forward, distance);
                bool hitsomething = false;
                for (int i = 0; i < hit.Length; i++)
                {
                    if (hit[i].collider != null && hit[i].collider.transform.root != this.transform.root)
                    {
                        MainCamera.transform.position = hit[i].point;
                        hitsomething = true;
                        break;
                    }
                }
